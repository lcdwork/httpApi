...
/**
 * @author: LCD
 * @date Create in: 16:40 2020/3/11
 * @description：
 * @modify:
 * @see: com.crplingxi.httpapi.webapp.action
 */
@ControllerAdvice
@RequestMapping("/empbmday")
public class EMpBmDayAction {

    @Autowired
    @Qualifier("EMpBmDayServiceImpl")
    private EMpBmDayService eMpBmDayService;
    ...
    @ResponseBody
    @RequestMapping()
    @Scheduled(cron = "${empbmday.cron}")
    public String putTest() {

        // 表名前缀
        String tableName = "E_MP_BM_DAY";

        // 拼接查询对象
        EMpBmDay eMpBmDay = new EMpBmDay();
        eMpBmDay.setTableName1(tableName + "_" + DateUtils.getLastYear());
        eMpBmDay.setTableName2(tableName + "_" + DateUtils.getNowYear());
        eMpBmDay.setSavedatetime(sendLogService.getLastTime(tableName));
        List<EMpBmDay> dataList = eMpBmDayService.findByWhere(eMpBmDay);
